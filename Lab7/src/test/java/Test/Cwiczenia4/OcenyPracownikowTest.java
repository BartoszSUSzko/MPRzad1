...
public class OcenyPracownikowTest {
    ...
    @Test
    void NajlepsiPracownicyNull() {
        PracownikManager manager = new PracownikManager();
        assertThrows(IllegalArgumentException.class,  () -> manager.najlepsiPracownicy());
    }

    @Test
    void SredniaOcenEmpty(){
        PracownikManager manager = new PracownikManager();
        Pracownik p1 = new Pracownik("A", "A", "@a", "FirmaA", Stanowiska.Prezes);

        assertThrows(IllegalArgumentException.class,  () -> p1.sredniaOcen());
    }
