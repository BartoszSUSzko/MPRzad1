...
public class AwansIPodwyzkaTest {
    ...
    @Test
    void testMatcheryNumeryczneHamcrest() {
        pracownik.setWynagrodzenie(9500);

        org.hamcrest.MatcherAssert.assertThat(pracownik.getWynagrodzenie(),
                allOf(
                        greaterThan(9000),
                        lessThan(10000),
                        not(equalTo(12000))
    ...
    @Test
    void testHamcrestKolekcje() {
        List<String> stanowiska = List.of("Stazysta", "Programista", "Manager");

        assertThat(stanowiska, hasSize(3));
        assertThat(stanowiska, hasItem("Manager"));
        assertThat(stanowiska, containsInAnyOrder("Manager", "Programista", "Stazysta"));
        assertThat(stanowiska, everyItem(notNullValue()));
    }


    @Test
    void testAssertJExtractingAndCollections() {
        Pracownik p2 = new Pracownik("a", "A", "@a", "FirmaA", Stanowiska.Programista);
        pracownikManager.dodajPracownika(p2);

        assertThat(pracownikManager.getListaPracownikow())
                .hasSize(2)
                .extracting(Pracownik::getImie)
                .containsExactlyInAnyOrder("Ktos", "a");
    }
